<div>
    <form method="post" action="{{ route('admin.job.update', ['jobPost' => $jobPost]) }}">
        @method('put')
        @csrf
        <div class="row">
            <div class="col-lg-12">
                <div class="form-group">
                    ...
                    <input class="form-control" type="text" readonly value="{{ $jobPost->company->title }}">
                    <input id="company_id" class="form-control" type="hidden" name="company_id" autocomplete="off" required value="{{ $jobPost->company->id }}">
                </div>
            </div>
            <div class="col-lg-12">
                <div class="form-group">
                    <label for="title">{{__('Job title')}}*</label>
                    <input id="title" class="form-control" type="text" name="title" autocomplete="off" required value="{{ $jobPost->title }}">
                </div>
            </div>
                ...
                <div class="form-group">
                    ...
                    <select id="job_type_id" name="job_type_id" required>
                        @foreach(\App\Models\JobType::get() as $jobType)
                            <option value="{{$jobType->id}}" @if($jobPost->job_type_id == $jobType->id) selected @endif>{{$jobType->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="col-lg-6 col-md-6">
                <div class="form-group">
                    ...
                    <div class="input-group date" id="datetimepicker">
                        <input id="deadline" type="text" class="form-control" placeholder="12/11/2021" name="deadline" autocomplete="off" value="{{ $jobPost->deadline->format('d/m/Y') }}">
            ...
            <div class="col-lg-6 col-md-6">
                <div class="form-group">
                    ...
                    <input id="salary" class="form-control" type="text" name="salary" autocomplete="off" placeholder="USD" value="{{ $jobPost->salary }}">
                </div>
            </div>
            <div class="col-lg-6 col-md-6">
                <div class="form-group">
                    <label for="vacancies">{{__('Vacancies')}}*</label>
                    <input id="vacancies" class="form-control" type="number" name="vacancies" autocomplete="off" required min="1" value="{{ $jobPost->vacancies }}">
                </div>
            </div>
            <div class="col-lg-6 col-md-6">
                <div class="form-group">
                    ...
                    <select id="experience_id" name="experience_id" required>
                        @foreach(\App\Models\Experience::get() as $experience)
                            <option value="{{$experience->id}}" @if($jobPost->experience_id == $experience->id) selected @endif>{{$experience->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="col-lg-6 col-md-6">
                <div class="form-group">
                    ...
                    <select id="tag" name="tag" required>
                        <option value="1" {{ old('tag') == 1 ? 'selected' : '' || $jobPost->tag == 1) selected }}>{{ __('Featured') }}</option>
                        <option value="2" {{ old('tag') == 2 ? 'selected' : '' || $jobPost->tag == 2) selected }}>{{ __('Urgent') }}</option>
                        <option value="3" {{ old('tag') == 3 ? 'selected' : '' || $jobPost->tag == 3) selected }}>{{ __('Immediate') }}</option>
                    </select>
                </div>
            </div>
                ...
                <div class="form-group">
                    ...
                    <p>
                        @foreach(\App\Models\Technology::get() as $technology)
                            {{__($technology->name)}}
                            <input type="checkbox" name="technologies[]" value="{{ $technology->id }}" class="mr-3" @if( Arr::exists($jobPost->technologies, $technology->id -1 ) ) checked @endif>
                        @endforeach
                    </p>
                </div>
            </div>
